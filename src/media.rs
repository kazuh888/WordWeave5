...
impl Speaker {
    ...
    pub fn stop(&mut self) {
        if let Some(t) = self.engine.as_mut() {
            let _ = t.stop();
        }
    }
    pub fn say(&mut self, text: &str, voice_id: &str, slow: bool) -> Result<(), String> {
        let t = self
            .engine
            .as_mut()
            .ok_or("Windowsの音声合成を初期化できません。英語の音声機能を追加してください。")?;
        let voices = t.voices().map_err(|e| e.to_string())?;
        let selected = if voice_id.is_empty() {
            voices
                .iter()
        ...
        let voice = selected.ok_or(
            "英語の音声が見つかりません。Windowsに英語の音声を追加し、アプリを再起動してください。",
        )?;
        ...
        t.set_rate(rate.clamp(t.min_rate(), t.max_rate()))
...
impl Recorder {
    pub fn start() -> Result<Self, String> {
        ...
        if channels == 0 || sample_rate > 192000 {
            ...
            ($sample:ty,$convert:expr) => {{
                ...
                device.build_input_stream(
                    ...
                    move |input: &[$sample], _: &cpal::InputCallbackInfo| {
                        append(input, channels, &d, cap, $convert)
                    },
                    move |e| {
