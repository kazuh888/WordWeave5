...
impl Config {
    ...
    pub fn chat(&self, payload: Value, images: Vec<Vec<u8>>) -> Result<wordweave5::chat_action::ChatReply, String> {
        let generated = codex::generate_with_settings(&self.exe, &self.cwd, &self.model,
    ...
    pub fn material(&self, request: wordweave5::material::Request, images: Vec<Vec<u8>>) -> Result<wordweave5::material::Draft, String> {
        ...
        let generated = codex::generate_with_settings(&self.exe,&self.cwd,&self.model,instructions,
    ...
    pub fn read_ink(&self, png: Vec<u8>) -> Result<String, String> {
        ...
        codex::generate(&self.exe,&self.cwd,&self.model,"画像内の日本語・英語をそのまま文字起こしする。丸や矢印は文字として解釈せず、取消線による変更は推測しない。推測で補完せず判読できない箇所は[?]。認識文字以外は出力しない。",vec![json!({"type":"text","text":"画像の文字をその言語のまま文字起こししてください。","text_elements":[]}),json!({"type":"image","url":url})],None,self.cancel.clone())
    }
    pub fn transcribe(&self, wav: Vec<u8>) -> Result<String, String> {
        ...
        codex::generate(&self.exe,&self.cwd,&self.model,"録音された日本語・英語をその言語のまま文字起こしする。翻訳・添削や推測で補完せず、聞き取れない箇所は[?]とする。認識した文字以外は出力しない。",vec![json!({"type":"text","text":"録音をその言語のまま文字起こししてください。"}),json!({"type":"audio","url":url})],None,self.cancel.clone())
    }
