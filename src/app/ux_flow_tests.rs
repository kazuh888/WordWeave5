...
fn short_course_respects_a_zero_daily_new_limit() {
    ...
    app.start(2);
    assert!(app.current.is_none(), "two-minute course must not bypass zero-new setting");
    assert!(app.session.is_none());
    drop(app);
    std::fs::remove_dir_all(root).unwrap();
}

fn study_frame(ctx: &egui::Context, app: &mut WordApp, events: Vec<egui::Event>) -> egui::FullOutput {
    ctx.run(egui::RawInput {
        screen_rect: Some(egui::Rect::from_min_size(egui::Pos2::ZERO, egui::vec2(1120.0, 2200.0))),
        events,
        ..Default::default()
    }, |ctx| {
    ...
    for pressed in [true, false] {
        study_frame(ctx, app, vec![
            ...
            egui::Event::PointerButton {
                pos: position, button: egui::PointerButton::Primary,
...
fn make_review_due(app: &mut WordApp, index: usize) -> String {
    ...
    app.progress.record(key.clone(), Grade::Good, false, "keyboard", now() - 172800,
        "2000-01-01", 600, false);
    key
}
...
fn check_current_answer(app: &mut WordApp, matching: bool) {
    ...
    app.answer = if matching { entry.answer().to_owned() } else { "a different natural answer".into() };
...
fn starting_from_home_resumes_without_discarding_the_current_answer() {
    ...
    assert_eq!(app.session.as_ref().unwrap().elapsed, Duration::from_secs(41));
...
fn finishing_presents_a_summary_and_never_double_credits_time() {
    ...
    app.finish();
    assert!(app.page == Page::Study, "finish must show a result, not return silently home");
    assert!(app.session.is_none());
    ...
    for shape in out.shapes { shape_text(&shape.shape, &mut text); }
...
fn failed_finish_keeps_session_for_export_instead_of_claiming_success() {
    ...
    assert!(app.session.is_some(), "retain session when persistence failed");
    assert_eq!(app.progress.study_seconds.get(&today()), Some(&65));
    drop(app);
    std::fs::remove_dir_all(root).unwrap();
}
...
fn reaching_the_time_budget_keeps_the_answer_until_an_explicit_grade() {
    ...
    assert_eq!(app.progress.reviews.len(), 1, "time alone must not record an answer");
    ...
    frame(&ctx, &mut app, false);
    assert_eq!(app.progress.reviews.len(), 2, "redrawing the result must not record again");
    drop(app);
    std::fs::remove_dir_all(root).unwrap();
}
...
fn continuing_respects_the_daily_new_limit_and_existing_review_due_dates() {
    ...
    assert!(app.progress.reviews[0].assisted, "immediate recall after introduction stays assisted");
    ...
    assert!(app.session.is_none(), "continuation is disabled when no eligible tasks remain");
    ...
    assert!(!app.current.as_ref().unwrap().introduce);
    assert!(app.session.as_ref().unwrap().queue.is_empty(), "the consumed new allowance must not reset");
    assert_eq!(app.progress.memories[&new_key].due, new_due);
    check_current_answer(&mut app, true);
    click_study_button(&ctx, &mut app, "自力でできた → 次へ");
    let summary = app.session_summary.as_ref().unwrap();
    assert_eq!(summary.answers, 1, "the second result excludes the first session");
    assert_eq!(summary.reviewed, 1);
    assert_eq!(summary.reason, session_end::EndReason::NoTasks);
    assert_eq!(app.progress.reviews.iter().filter(|review| review.date == today() && review.first).count(), 1);
    drop(app);
    std::fs::remove_dir_all(root).unwrap();
}
...
fn zero_answer_manual_end_and_no_eligible_tasks_have_distinct_results() {
    ...
    assert_eq!((summary.answers, summary.independent, summary.reviewed), (0, 0, 0));
...
fn summary_counts_session_answers_unique_reviews_and_self_assessed_success() {
    ...
    app.session.as_mut().unwrap().queue.push_back(Task { index: 0, skill: Skill::Recall, introduce: false });
    click_study_button(&ctx, &mut app, "確認した・別の問題のあとで思い出す");
    assert_eq!(app.key(), repeated_key);
    check_current_answer(&mut app, true);
    app.grade(Grade::Good);
    ...
    let summary = app.session_summary.as_ref().unwrap();
    assert_eq!(summary.answers, 4, "historical records are outside this session");
    assert_eq!(summary.independent, 3, "self-assessed success counts; assisted success does not");
    assert_eq!(summary.reviewed, 2, "repeated reviews count once and the first answer is excluded");
    assert_eq!(summary.expressions.len(), 3);
    assert_eq!(summary.seconds, 73, "active study time is not the sum of Review.seconds");
    assert_eq!(summary.next_due, app.progress.memories.values().map(|memory| memory.due).min());
    assert_eq!(app.progress.study_seconds.get(&today()), Some(&73));
    let output = study_frame(&ctx, &mut app, vec![]);
    let mut text = String::new();
    for shape in output.shapes { shape_text(&shape.shape, &mut text); }
    assert!(text.contains("ヒントなしでできたと記録：3回（自己評価を含む）"), "{text}");
    drop(app);
    std::fs::remove_dir_all(root).unwrap();
}
...
fn end_confirmation_pauses_time_and_can_return_to_the_unrecorded_answer() {
    ...
    frame(&ctx, &mut app, false);
    assert_eq!(app.session.as_ref().unwrap().elapsed, Duration::from_secs(17));
    assert_eq!(app.key(), key);
    assert_eq!(app.progress.reviews.len(), 1);
    ...
    app.last_frame = Instant::now() - Duration::from_secs(2);
    frame(&ctx, &mut app, false);
    assert_eq!(app.session.as_ref().unwrap().elapsed, Duration::from_secs(18));
    click_study_button(&ctx, &mut app, "ここで終える");
    click_study_button(&ctx, &mut app, "未記録の回答を破棄して終了");
    assert!(app.session.is_none());
    assert_eq!(app.progress.reviews.len(), 1, "discarding must not fabricate a grade");
