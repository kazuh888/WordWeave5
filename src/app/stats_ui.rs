...
fn record_summary(progress: &Progress, range: Option<(&str, &str)>) -> RecordSummary {
    ...
    for review in progress.reviews.iter().filter(|review| includes(&review.date)) {
        summary.answers += 1;
        if review.first {
    ...
    summary.seconds = progress.study_seconds.iter()
...
impl WordApp {
    pub(super) fn stats(&mut self, ui: &mut egui::Ui) {
        ux::heading(ui, "学習の記録", "学習した時間と、取り組んだ表現を振り返る。");
        let total = record_summary(&self.progress, None);
        ux::metric_group(ui, |ui| {
            ux::metric(ui, "記録した学習時間", ux::duration(total.seconds), "これまでの合計");
            ux::metric(ui, "記録した回答", format!("{}回", total.answers), "同じ項目への再回答を含む");
            ux::metric(ui, "取り組んだ練習項目", format!("{}項目", total.items), "同じ教材でも練習形式ごとに数える");
        });
        ui.add_space(12.0);
        if total.answers == 0 {
            ux::panel(ui, true, |ui| {
        ...
        ux::panel(ui, false, |ui| {
            ui.strong("回答の内訳");
            ux::metric_group(ui, |ui| {
                ux::metric(ui, "初めての回答", format!("{}回", total.new_answers), "回答時点で初回だった記録");
                ux::metric(ui, "復習としての回答", format!("{}回", total.review_answers), "2回目以降の記録");
            });
    ...
    fn recent_activity(&self, ui: &mut egui::Ui) {
        ...
        let days: Vec<_> = (0..7).rev().map(|days| {
            let date = (today - chrono::Duration::days(days)).format("%Y-%m-%d").to_string();
        ...
        let longest = days.iter().map(|(_, day)| day.seconds).max().unwrap_or(0).max(1);
        ux::panel(ui, false, |ui| {
            ui.strong("直近7日の学習");
            ui.label(format!("{} / {}回答 / {}練習項目", ux::duration(week.seconds), week.answers, week.items));
            for (date, day) in &days {
                ui.horizontal_wrapped(|ui| {
                    ui.strong(date);
                    ui.label(format!("{} · {}回答 · 新規{} / 復習{}", ux::duration(day.seconds), day.answers, day.new_answers, day.review_answers));
                });
                ui.add(egui::ProgressBar::new(day.seconds as f32 / longest as f32)
                    .desired_width(ui.available_width()).desired_height(8.0));
                ui.add_space(4.0);
            }
    ...
    fn record_details(&self, ui: &mut egui::Ui) {
        ux::panel(ui, false, |ui| {
            ...
            ui.collapsing("練習の種類別の記録", |ui| {
                for skill in Skill::ALL {
                    let suffix = format!(":{}", skill.code());
                    let answers = self.progress.reviews.iter().filter(|r| r.key.ends_with(&suffix)).count();
                    let items = self.progress.memories.keys().filter(|key| key.ends_with(&suffix)).count();
                    ui.label(format!("{}：{answers}回答 / 復習状態のある項目 {items}件", skill.label()));
                }
                ui.small("回答数は過去の履歴、復習状態のある項目数は現在保持している状態から集計する。");
            });
            ui.collapsing("入力方法別の回答記録", |ui| {
    ...
    fn writing_record_details(&self, ui: &mut egui::Ui) {
        ux::panel(ui, false, |ui| {
            ...
            for (index, log) in self.progress.writing_logs.iter().enumerate().rev().take(20) {
                ...
                egui::CollapsingHeader::new(format!("{time} / {}", if log.revision { "書き直し" } else { "初回回答" }))
                    .id_salt(("writing-record", index)).show(ui, |ui| {
...
mod tests {
    ...
    fn review(key: &str, date: &str, first: bool) -> Review {
        Review {
            key: key.into(), at: 0, date: date.into(), grade: Grade::Good,
            assisted: false, method: "keyboard".into(), first,
            elapsed_days: 0.0, seconds: 999, self_assessed: true,
        }
    }

    #[test]
    fn empty_records_have_zero_activity() {
        assert_eq!(record_summary(&Progress::default(), None), RecordSummary::default());
    }
    ...
    fn date_range_is_inclusive_and_uses_recorded_study_time() {
        ...
        assert_eq!(summary, RecordSummary {
            answers: 2, new_answers: 0, review_answers: 2, items: 1, seconds: 20,
        });
    }

    #[test]
    fn repeated_answers_count_once_per_practice_item_without_inferring_mastery() {
        let mut progress = Progress { reviews: vec![
        ...
        assert_eq!(record_summary(&progress, None), RecordSummary {
            answers: 3, new_answers: 2, review_answers: 1, items: 2, seconds: 0,
        });
    }
