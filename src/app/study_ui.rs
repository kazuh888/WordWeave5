...
impl WordApp {
    ...
    pub(super) fn study_feedback(&mut self, ui: &mut egui::Ui, entry: &Entry, skill: Skill) {
            ...
            match self.matched {
                ...
                Some(false) => {
                    ui.colored_label(
                        ...
                        "登録例とは異なる。自然な別解かどうか、意味・文法・場面を確認する。",
                    );
                }
                None => {
                    ui.label("解答例と比べて、自分の回答を評価する。");
                }
            }
        ...
        ui.add_space(8.0);
        let guidance = ui.add(
            egui::Label::new("自分で評価して次へ進む（Tabで評価を選び、Enterで確定）")
        ...
        ui.add_enabled_ui(self.pending.is_none(), |ui| {
            ui.horizontal_wrapped(|ui| {
                for (grade, label, enabled) in [
                    ...
                    if ui
                        .add_enabled(enabled, crate::app::controls::Button::new(label).wrap())
                        .clicked()
                    {
                        self.grade(grade);
                        break;
        ...
        crate::app::controls::CollapsingHeader::new("詳しい解説・例文を確認")
            .id_salt(("study-details", self.key()))
            .show(ui, |ui| {
                self.card(ui, entry);
                if matches!(skill, Skill::Usage | Skill::Sentence) {
                    ui.label(&entry.explanation);
                }
            });
        crate::app::controls::CollapsingHeader::new("AIに使い方を確認する（任意）")
            ...
            .show(ui, |ui| {
                ...
                if ui
                    .add_enabled(
                        ...
                        crate::app::controls::Button::new("回答について確認する（Codexへ送信）")
                            .wrap(),
                    )
                    .clicked()
                {
