...
impl WordApp {
    ...
    pub(super) fn set_conversation_deleted(
        &mut self,
        id: &str,
        deleted: bool,
    ) -> Result<(), String> {
        ...
        next.set_chat_deleted(id, deleted).map_err(|error| {
            operation.fail(DiagnosticStage::Edit, DiagnosticError::InvalidData);
            error
        })?;
        self.storage
            .as_ref()
        ...
        operation.event(
            ...
            if deleted {
                DiagnosticEvent::Deleted
            } else {
                DiagnosticEvent::Restored
            },
        );
        self.progress = next;
        self.dirty = false;
        ...
        self.message = if deleted {
            "会話をごみ箱へ移動した。教材・復習記録・媒体原本は保持している。"
        } else {
            "会話を復元した。"
        }
        .into();
    ...
    pub(super) fn conversation_trash_windows(&mut self, ctx: &egui::Context) {
        ...
        if let Some(id) = self.pending_chat_delete.clone() {
            ...
            egui::Window::new("会話の削除を確認")
                .collapsible(false)
                .show(ctx, |ui| {
                    ux::dialog_body(ui);
                    ui.label(format!("「{title}」をごみ箱へ移動する。"));
                    ui.label("復元可能。教材・復習記録・媒体原本・Codex側の履歴は削除しない。");
                    if let Err(reason) = self.chat_change_allowed() {
                        ui.label(reason);
                    }
                    ui.horizontal(|ui| {
                        apply = ui
                            .add_enabled(allowed, crate::app::controls::Button::new("ごみ箱へ移動"))
                            .clicked();
                ...
                for chat in chats {
                    ui.push_id(&chat.id, |ui| {
                        ui.horizontal_wrapped(|ui| {
                            ui.label(&chat.title);
                            if ui.ww_button("内容を見る").clicked() {
                                self.viewed_trash_id = Some(chat.id.clone());
                            }
                            if ui
                                .add_enabled(allowed, crate::app::controls::Button::new("復元"))
                                .clicked()
                            {
                                restore = Some(chat.id.clone());
                            }
                        });
                        ui.separator();
                    });
        ...
        if let Some(id) = self.viewed_trash_id.clone() {
            ...
            egui::Window::new("削除した会話（読み取り専用）")
                ...
                .show(ctx, |ui| {
                    ...
                    if !chat.memo.is_empty() {
                        ui.label(format!("引き継ぎメモ：{}", chat.memo));
                    }
                    if !chat.draft.is_empty() {
                        ui.label(format!("未送信の下書き：{}", chat.draft));
                    }
                    for (index, exchange) in chat.exchanges.iter().enumerate() {
                        ui.separator();
                        ui.strong(format!("往復 {}：あなた", index + 1));
                        ui.label(&exchange.question);
                        ui.strong("Codex");
                        ui.label(&exchange.answer);
                        ui.small(exchange.execution.label());
                    }
                    ui.separator();
                    ui.strong("保存済みの添付（下書き・履歴）");
                    for (index, attachment) in chat
                        .draft_attachments
                        .iter()
                        .chain(chat.exchanges.iter().flat_map(|e| &e.attachments))
                        .enumerate()
                    {
                        ui.push_id(index, |ui| {
                            ui.label(&attachment.source_text);
                            if let Some(text) = &attachment.transcript {
                                ui.label(text);
                            }
                            if attachment.original.kind == wordweave5::assets::AssetKind::AudioWav
                                && ui
                                    ...
                                    .clicked()
                            {
                                play = Some(attachment.original.clone());
                            }
                            if let Some(image) = &attachment.image {
                                if ui.ww_button("注釈画像").clicked() {
