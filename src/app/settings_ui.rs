...
impl WordApp {
    pub(super) fn settings(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ux::heading(ui, "設定", "学習しやすい量・入力方法・音声を選ぶ。接続やバックアップの設定は下にまとめてある。");
        ...
        ux::panel(ui, false, |ui| {
            ...
            ui.add(egui::Slider::new(&mut self.progress.settings.ai_daily_limit, 0..=1000));
            ui.label("1回に生成する例文数");
            ui.add(egui::Slider::new(&mut self.progress.settings.examples_per_word, 3..=12));
            ui.small("生成・添削の上限は語数ではなく試行回数である。ChatGPT契約側の利用枠とは別の設定。");
        });
        ui.add_space(12.0);
    ...
    fn learning_settings_panel(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        ui.add(egui::Slider::new(&mut self.progress.settings.minutes, 1..=30));
        ui.label("新規項目の1日上限");
        ui.add(egui::Slider::new(&mut self.progress.settings.new_per_day, 0..=20));
        ...
        let mut tags: Vec<String> = self.deck.iter().map(|e| e.tag.clone())
            .collect::<std::collections::BTreeSet<_>>().into_iter().collect();
        tags.insert(0, "すべて".into());
        ui.label("出題する場面・分類");
        egui::ComboBox::from_id_salt("topic")
            .width(ui.available_width().min(440.0)).truncate()
        ...
        ui.add_space(8.0);
        ui.label("画面の拡大率");
        if ui.add(egui::Slider::new(&mut self.progress.settings.font_scale, 0.8..=1.6)).changed() {
    ...
    fn voice_settings_panel(&mut self, ui: &mut egui::Ui) {
        ...
        let current_voice = self.speaker.voices.iter()
            .find(|v| v.0 == self.progress.settings.voice_id)
            .map(|v| v.1.clone()).unwrap_or_else(|| "英語の音声を自動選択".into());
        ...
        if ui.add(egui::Slider::new(&mut rate, 0.5..=4.0).step_by(0.1)).changed() {
    ...
    fn connection_settings_panel(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label("Codex CLIをインストールし、ターミナルで codex login を実行してChatGPTでログインする。");
        ui.small("APIキーは使用しない。生成はChatGPT契約の利用枠を使用し、上限到達時は停止する。");
        ui.add_space(8.0);
        ui.label("Codex実行ファイル");
        ui.add(egui::TextEdit::singleline(&mut self.progress.settings.codex_path)
            .desired_width(ui.available_width().min(720.0)));
        if ui.button("Codex実行ファイルを選択").clicked() {
            if let Some(path) = rfd::FileDialog::new().add_filter("Codex", &["exe", "cmd", "bat"]).pick_file() {
        ...
        ui.add(egui::TextEdit::singleline(&mut self.progress.settings.codex_model)
            .desired_width(ui.available_width().min(440.0)));
        self.effort_settings(ui);
        ui.small("ここは次回の要求設定。実際のモデル・effortは、Codexの返却後に画面下部へ表示する。");
        ui.label(self.connection_label());
        if ui.add_enabled(self.pending.is_none(), egui::Button::new("接続・ChatGPT認証を確認")).clicked() {
    ...
    fn backup_settings_panel(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal_wrapped(|ui| {
            if ui.button("教材をTSVに書き出す").clicked() {
                if let Some(path) = rfd::FileDialog::new().set_file_name("wordweave-deck.tsv")
                    .add_filter("TSV", &["tsv"]).save_file() {
                    self.message = store::atomic_write(&path, model::deck_text(&self.deck).as_bytes())
                        .map(|_| "教材を書き出した。編集後は取り込みで反映できる。".into()).unwrap_or_else(|e| e);
                }
            }
        ...
        if let Some(storage) = &self.storage {
            ui.add(egui::Label::new(RichText::new(format!("保存先：{}", storage.dir.display())).small()).wrap());
        }
