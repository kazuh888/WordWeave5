...
impl WordApp {
    pub(super) fn effort_choices(&self) -> Option<&wordweave5::effort::ModelEffort> {
        ...
        if path != self.progress.settings.codex_path.trim() { return None; }
        models.iter().find(|m| m.model == self.progress.settings.codex_model.trim())
    }

    pub(super) fn effort_settings(&mut self, ui: &mut egui::Ui) {
        if ui.add_enabled(self.pending.is_none() && self.recorder.is_none() && self.fatal.is_none(),
            egui::Button::new("モデル・effort候補を取得")).clicked() {
        ...
        egui::ComboBox::from_id_salt("codex-effort").selected_text(selected).show_ui(ui, |ui| {
            ...
            if let Some(choices) = choices {
                for choice in choices {
                    ui.selectable_value(&mut self.progress.settings.codex_effort, choice.effort.clone(), &choice.effort)
        ...
        if let Some(default) = model.as_ref().and_then(|m| m.default_effort.as_ref()) {
            ui.small(format!("サーバーが示すこのモデルの既定effort：{default}（今回の実行値ではない）"));
        }
        if choices.is_none() {
            ui.label("このモデルのeffort候補は未取得。候補を取得し、モデルを選択すると指定できる。");
        }
        if !self.progress.settings.codex_effort.is_empty()
            && !choices.is_some_and(|c| c.iter().any(|e| e.effort == self.progress.settings.codex_effort)) {
    ...
    fn load_effort_catalog(&mut self) {
        ...
        std::thread::spawn(move || {
            let result = wordweave5::codex::list_model_efforts(&config.exe, &config.cwd, config.cancel)
            ...
            if let Some(text) = &self.diagnostic_export {
                if text.is_empty() { ui.label("診断ログはまだない。"); }
                    ...
                    ui.horizontal_wrapped(|ui| {
                        if ui.button("表示中のログをコピー").clicked() { ui.ctx().copy_text(text.clone()); }
                        if ui.button("表示中のログをファイルに保存").clicked() {
