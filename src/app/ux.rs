...
pub(super) fn heading(ui: &mut egui::Ui, title: &str, description: &str) {
    ...
    if !description.is_empty() { ui.label(RichText::new(description).color(MUTED)); }
    ui.add_space(16.0);
}

pub(super) fn panel<R>(ui: &mut egui::Ui, tint: bool, content: impl FnOnce(&mut egui::Ui) -> R) -> R {
...
pub(super) fn primary(ui: &mut egui::Ui, text: impl Into<String>, enabled: bool) -> egui::Response {
    ...
    ui.add_enabled(enabled, egui::Button::new(
        RichText::new(text.into()).size(20.0).strong().color(Color32::WHITE))
        .fill(ACCENT).min_size(egui::vec2(width, 48.0)).wrap())
}
...
pub(super) fn metric(ui: &mut egui::Ui, label: &str, value: impl Into<String>, note: &str) {
    ui.group(|ui| {
        ...
        if !note.is_empty() { ui.small(note); }
    });
}
...
pub(super) fn metric_group(ui: &mut egui::Ui, content: impl FnOnce(&mut egui::Ui)) {
    if ui.available_width() < 700.0 { ui.vertical(content); }
    else { ui.horizontal_wrapped(content); }
}
