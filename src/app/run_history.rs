...
impl WordApp {
    pub(super) fn run_history(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("Codex実行記録・結果の再取得").open(&mut open).default_width(820.0).show(ctx, |ui| {
            ...
            if ui.add_enabled(self.pending.is_none(), egui::Button::new("実行記録を更新")).clicked() {
                self.refresh_runs();
            }
            ...
            egui::ScrollArea::vertical().max_height(520.0).show(ui, |ui| {
                for record in &self.run_records {
                    ...
                    egui::CollapsingHeader::new(format!("{} / {}", date, record.outcome.label())).id_salt(&record.id).show(ui, |ui| {
                        ...
                        if let Some(text) = &record.response {
                            if ui.button("本文をコピー").clicked() { ui.ctx().copy_text(text.clone()); }
                            let mut shown = text.as_str();
                            ui.add(egui::TextEdit::multiline(&mut shown).desired_rows(8).desired_width(f32::INFINITY));
                        } else if ui.add_enabled(self.pending.is_none() && record.turn_id.is_some(), egui::Button::new("この往復の結果を再取得（生成しない）")).clicked() {
